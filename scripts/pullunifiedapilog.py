...
LOG_GROUP = "/aws/lambda/RoutingAPIStack-RoutingLamb-RoutingLambda2C4DF0900-SqD4I5W2VxTY"
...
def main():
    ...
    parser.add_argument("--hours", type=int, default=72, help="回溯小时数，默认72小时")
    ...
    try:
        evts = fetch_logs(session, LOG_GROUP, args.id, args.hours, args.limit)
        if not evts:
            print("⚠️ 未找到匹配日志")
            return
        print(f"✅ 获取到 {len(evts)} 条匹配日志\n")
        for i, e in enumerate(evts, 1):
            print(fmt_event(e, i))
        print(f"📈 总计: {len(evts)} 条")
